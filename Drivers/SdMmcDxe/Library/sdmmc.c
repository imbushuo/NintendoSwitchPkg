...
{
	ZeroMem(storage, sizeof(sdmmc_storage_t));
	storage->sdmmc = sdmmc;
	...
	u8 ext_csd[512];
	ZeroMem(ext_csd, sizeof(ext_csd));
...
static int _sd_storage_get_op_cond(sdmmc_storage_t *storage, int is_version_1, int supports_low_voltage)
{
	...
	while (1)
	{
		...
		if (cond & MMC_CARD_BUSY)
		{
			if (cond & SD_OCR_CCS) storage->has_sector_access = 1;
		...
		if (get_tmr() > timeout) break;

		MicroSecondDelay(10000); // Needs to be at least 10ms for some SD Cards
	}
