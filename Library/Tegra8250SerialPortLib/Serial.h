#ifndef _TEGRA_8250_SERIALPORT_LIB_H_
#define _TEGRA_8250_SERIALPORT_LIB_H_

// Serial.h: a serial struct helper

// This is UARTB
#define CONFIG_CONSOLE_SERIAL_TEGRA210_UART_ADDRESS 0x70006040

struct tegra210_uart {
	union {
		UINT32 thr; // Transmit holding register.
		UINT32 rbr; // Receive buffer register.
		UINT32 dll; // Divisor latch lsb.
	};
	union {
		UINT32 ier; // Interrupt enable register.
		UINT32 dlm; // Divisor latch msb.
	};
	union {
		UINT32 iir; // Interrupt identification register.
		UINT32 fcr; // FIFO control register.
	};
...
#define write8(a, v) MmioWrite8((UINTN)(a), (UINT8)(v))
#define read8(a) MmioRead8((UINTN)(a))
